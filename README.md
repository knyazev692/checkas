...
│   ├── api_server.py          # FastAPI сервер (порт 8083)
│   ├── start_api_server.py    # Запуск сервера
│   └── database_config.py     # Конфигурация SQLite БД
│
...
```bash
# Запуск FastAPI сервера на порту 8083
python server/api_server.py

# Или через скрипт запуска
python server/start_api_server.py
```
...
```bash
# Интерактивная панель управления DND
python examples/admin_dnd_control.py

# Получение статуса конкретного клиента
python examples/get_client_status.py HOSTNAME

# Мониторинг клиента в реальном времени
python examples/get_client_status.py HOSTNAME monitor
```
...
API_SERVERS = [
    "http://172.16.99.76:8083",  # Основной сервер
    "http://172.16.99.77:8083",  # Резервный сервер 1
    "http://172.16.99.78:8083",  # Резервный сервер 2
]
